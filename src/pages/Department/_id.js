...
export default function Department() {
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        const res = await axios.get(`${api}/chi-tiet-phong-ban/${id}`);
  ...
  return (
    <Grid container spacing={2}>
      <Grid item lg={12}>
        <CardTable title={`Danh sach nhan vien ${department.tenphongban}`}>
          <TableContainer sx={{ maxHeight: "450px", overflow: "auto" }}>
            <Table stickyHeader>
              ...
              <TableBody>
                {department?.nhanvien?.length > 0 &&
                  department?.nhanvien.map((item, index) => (
